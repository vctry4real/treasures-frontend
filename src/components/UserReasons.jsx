...
const CardListItem = ({ cardReason }) => {
  return (
    <div className="flex flex-row border-2 border-grey p-4 gap-4 mb-4 max-w-700 w-full shadow-3xl rounded-md hover:grey-200">
      <div className="flex justify-between item-center">
        ...
        <input
          type="check"
          name="reason"
          value={cardReason.isSelected}
          id="parent"></input>
      </div>
      <div className="flex flex-col leading-normal">
        <h3 className="font-bold text-md">{cardReason.title}</h3>
      </div>
      </div>
    </div>
  );
  ...
  return ( 
    <div className="min-w-300 justify-center">
      {reasons.map((item, index) => {
        return <CardListItem key={index} cardReason={item} />;
      })}
    </div>
  );
};
const UserReasons = () => {
  ...
  return (
    // Whole screen
    <div className="bg-primary w-full h-screen flex items-center justify-center setEmail ">
      {/* LHS image */}
      <div className="w-full flex flex-col md:flex-row items-center justify-center lg:h-screen">
        ...
        <div className="bg-white rounded-md h-screen flex flex-col items-center justify-center w-screen md:w-1/2 lg:w-2/3">
          ...
          <p>Select your primary reason</p>
          <CardList />
          <p className="mr-1">
          Continue
          {/* {email} */}
...
<p className="mr-1">This allows us deliver a more personalied experience for you.</p>;
